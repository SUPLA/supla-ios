...
final class InsertChannelConfigUseCaseTests: UseCaseTest<Void> {
    ...
    func test_shouldInsertContainerConfig() {
        // given
        let remoteId: Int32 = 231
        let crc32: Int64 = 123
        let profile = AuthProfileItem.mock()
        let channel = SAChannel.mock()
        let config = SuplaChannelContainerConfig.mock(remoteId: remoteId, crc32: crc32)
        let channelConfig = SAChannelConfig.mock()
        ...
        XCTAssertEqual(channelConfig.channel, channel)
        XCTAssertEqual(channelConfig.config_crc32, crc32)
        XCTAssertEqual(channelConfig.config_type, Int32(ChannelConfigType.container.rawValue))
    }
    ...
    func test_shouldUpdateContainerConfig() {
        // given
        let remoteId: Int32 = 231
        let crc32: Int64 = 123
        let config = SuplaChannelContainerConfig.mock(remoteId: remoteId, crc32: crc32)
