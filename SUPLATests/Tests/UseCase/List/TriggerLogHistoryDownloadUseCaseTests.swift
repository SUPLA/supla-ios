...
final class TriggerLogHistoryDownloadUseCaseTests: XCTestCase {
    ...
    func test_shouldSkipChannelsWithNoAggregation() async {
        ...
        // then
        XCTAssertEqual(channelRepository.findChannelsByMock.parameters.map { $0.0 }, Array(repeating: profileId, count: 7))
    ...
    func test_shouldDownloadWhenEntriesAreMissingAndSkipRepeatedInvokeWithinLogsInterval() async {
        ...
        // then
        XCTAssertEqual(channelRepository.findChannelsByMock.parameters.map { $0.0 }, Array(repeating: profileId, count: 14))
    ...
    func test_shouldDownloadWhenEntriesAreStaleAndSkipRepeatedInvokeWithinRefreshInterval() async {
        ...
        let firstDate = Date(timeIntervalSince1970: 1_000_000)
        let secondDate = firstDate.addingTimeInterval(60)
        let staleMeasurement = makeMeasurement(date: firstDate.addingTimeInterval(-3601))
        let profile = makeProfile(id: profileId)
    ...
    private func makeChannel(profileId: Int32, serverId: Int32, remoteId: Int32, flags: Int64 = 0) -> SAChannel {
    ...
    private func makeMeasurement(date: Date) -> SAImpulseCounterMeasurementItem {
