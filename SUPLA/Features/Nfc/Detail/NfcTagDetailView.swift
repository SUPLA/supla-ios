...
extension NfcTagDetailFeature {
    ...
    struct View: SwiftUI.View {
        ...
        private func ActionText() -> some SwiftUI.View {
            if let actionName = viewState.actionId?.name,
               let subjectName = viewState.subjectName,
               let profileName = viewState.profileName
            {
                Text("\(actionName) - \(subjectName) (\(profileName))")
                    .fontBodyLarge()
            } else if let actionName = viewState.actionId?.name,
                      let subjectName = viewState.subjectName
            {
                Text("\(actionName) - \(subjectName)")
                    .fontBodyLarge()
            } else {
                HStack {
                    Image(.Icons.warning)
                ...
                if (viewState.lastReadingItems.isEmpty) {
                    ...
                    ForEach(viewState.lastReadingItems.indices, id: \.self) { idx in
                        ...
                        HStack(alignment: .center, spacing: Distance.tiny) {
                            ...
                            Text(reading.result.iconText)
                                .fontBodyMedium()
                                .textColor(reading.result.iconColor)
                            Text(reading.result.text)
                                .fontBodyMedium()
                        }
                        .fixedSize()
                    }
...
#Preview("Locked") {
    NfcTagDetailFeature.View(
        viewState: NfcTagDetailFeature.ViewState(
            ...
            tagLocked: true,
            lastReadingItems: [
                NfcCallItemDto(date: .now, result: .success),
                NfcCallItemDto(date: .now.advanced(by: -120), result: .failure),
                NfcCallItemDto(date: .now.advanced(by: -12000), result: .actionMissing),
                NfcCallItemDto(date: .now.advanced(by: -92000), result: .success)
            ]
        ),
        delegate: nil
    )
