...
private enum DefaultDimens {
    ...
    static var ratio: CGFloat { width / (height + 40) }
...
final class RoofWindowView: BaseWindowView {
    ...
    private var openedOffset: CGFloat {
    ...
    private func drawPath(_ context: CGContext, fillColor: UIColor, withShadow: Bool = false, _ pathProducer: () -> CGPath) {
        context.beginPath()
        context.addPath(pathProducer())
        context.setFillColor(fillColor.cgColor)
        if (withShadow) {
            context.setShadow(offset: Dimens.Shadow.offset, blur: 6)
        } else {
    ...
    private func dynamicTransformation() -> CGAffineTransform {
        let firstTransformation = CATransform3DMakeRotation(degreesToRadians(windowRotationY), 0, 1, 0)
        let secondTransformation = CATransform3DRotate(firstTransformation, degreesToRadians(windowRotationX + openedOffset), 1, 0, 0)
        return CATransform3DGetAffineTransform(secondTransformation)
    }
