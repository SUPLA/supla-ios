...
class BaseDetailVC<S: ViewState, E: ViewEvent, VM: BaseDetailVM<S, E>>: SuplaTabBarController<S, E, VM>, NavigationItemProvider {
    ...
    private func containerGeneralDetail() -> UIViewController {
        let vc = ContainerGeneralFeature.ViewController.create(channelId: item.remoteId)
        vc.tabBarItem = UITabBarItem(
            title: settings.showBottomLabels ? Strings.StandardDetail.tabGeneral : nil,
            image: .iconGeneral,
            tag: DetailTabTag.General.rawValue
    ...
    private func rgbDetail() -> UIViewController {
        let vc = RgbDetailFeature.ViewController.create(itemBundle: item)
        vc.tabBarItem = UITabBarItem(
            title: settings.showBottomLabels ? Strings.StandardDetail.tabRgb : nil,
            image: .iconRgb,
            tag: DetailTabTag.Rgb.rawValue
        )
        vc.tabBarItem.selectedImage = .iconRgbSelected
        return vc
    }
