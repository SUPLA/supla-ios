...
private let HOUSE_PATH_D = "M173.5 71.5072V54.0001C173.501 51.6089 172.897 49.2463 171.73 47.0772C170.563 44.9081 168.862 42.9848 " +
...
private let PATH = Path.svg(HOUSE_PATH_D)

extension GraphicsContext {
    func drawHousePath(scale: CGFloat, inRect: CGRect) {
        let path = PATH
            .applying(CGAffineTransform(scaleX: scale, y: scale))
            .applying(CGAffineTransform(translationX: inRect.minX, y: inRect.minY))
        
        stroke(
            path,
            with: .color(Color.Supla.outline),
