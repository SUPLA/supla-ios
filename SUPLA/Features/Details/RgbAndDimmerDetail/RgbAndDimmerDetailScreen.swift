...
struct RgbAndDimmerDetailFeature {}

extension RgbAndDimmerDetailFeature {
    struct Screen: SwiftUI.View {
        ...
        @StateObject private var viewModel: RgbAndDimmerDetailVM
        
        init(item: ItemBundle, pages: [DetailPage]) {
            self.item = item
            self.pages = pages
            self._viewModel = StateObject(wrappedValue: RgbAndDimmerDetailVM(item: item))
        }
