...
final class UpdateEventsManagerImpl: UpdateEventsManager {
    ...
    private let channelUpdatesSubject = PublishRelay<Void>()
    private let groupUpdatesSubject = PublishRelay<Void>()
    private let sceneUpdatesSubject = PublishRelay<Void>()
    private let updatesSubject = PublishRelay<Id>()
    ...
    func emitSceneUpdate(sceneId: Int) {
        ...
        updatesSubject.accept(Id(type: .scene, id: sceneId))
    }

    func emitChannelUpdate(remoteId: Int) {
        let subject = getSubjectForChannel(channelId: remoteId)
        subject.accept(subject.value + 1)
        updatesSubject.accept(Id(type: .channel, id: remoteId))
        ...
        if let rootParents = channelToRootRelationHolderUseCase.getParent(for: Int32(remoteId)) {
            rootParents.forEach { rootParent in
                ...
                updatesSubject.accept(Id(type: .channel, id: Int(rootParent)))
            }
        }
    }
    ...
    func emitGroupUpdate(remoteId: Int) {
        ...
        subject.accept(subject.value + 1)
        updatesSubject.accept(Id(type: .group, id: remoteId))
    }
