...
protocol UserStateHolder {
    ...
    func getDimmerSelectorType(profileId: Int32, remoteId: Int32) -> DimmerDetailBase.DimmerSelectorType
    func setDimmerSelectorType(_ type: DimmerDetailBase.DimmerSelectorType, profileId: Int32, remoteId: Int32)
...
final class UserStateHolderImpl: UserStateHolder {
    ...
    func getImpulseCounterSettings(profileId: Int32, remoteId: Int32) -> ImpulseCounterSettings {
        ...
        if let data = userDefaults.data(forKey: key) {
            do {
                let decoder = JSONDecoder()
                return try decoder.decode(ImpulseCounterSettings.self, from: data)
            } catch {
                let errorString = String(describing: error)
                SALog.error("Could not decode impulse counter settings \(errorString)")
            }
        }
        
        return ImpulseCounterSettings.defaultSettings()
    }
    ...
    func getDimmerSelectorType(profileId: Int32, remoteId: Int32) -> DimmerDetailBase.DimmerSelectorType {
        ...
        return DimmerDetailBase.DimmerSelectorType.from(userDefaults.integer(forKey: key))
    }
