...
struct DoubleIconValueListItemView: View {
    ...
    var body: some View {
        ListItemScaffold(
            ...
            ListItemMainRow(spacing: scaleFactor <= 1 ? 4 : Distance.tiny) {
                if (scaleFactor <= 1) {
                    ListItemIcon(iconResult: item.base.icon)
                    if let value = item.base.value {
                        ListItemValue(value: value)
                    }
                    if let secondIcon = item.secondIcon {
                        ListItemIcon(iconResult: secondIcon)
                    }
                    if let secondValue = item.secondValue {
                        ListItemValue(value: secondValue)
                    }
                } else {
                    VStack(alignment: .center, spacing: Distance.tiny) {
                        HStack {
                            ListItemIcon(iconResult: item.base.icon)
                                .environment(\.scaleFactor, 0.6)
                            if let value = item.base.value {
                                ListItemValue(value: value)
                            }
                        }
    ...
    VStack(spacing: 1) {
        DoubleIconValueListItemView(
            item: DoubleValueListItem(
                ...
                secondIcon: .originalSuplaIcon(name: .Icons.fncHumidity),
                secondValue: "50"
            ),
            onInfoClick: {},
            onIssueClick: { _ in },
