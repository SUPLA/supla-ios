...
-(void)setChannel:(SAChannel *)channel {
    ...
    if ( [channel.func intValue] == SUPLA_CHANNELFNC_THERMOMETER ) {
        ...
        [self.temp setText:[channel isOnline] ? [NSString stringWithFormat:@"%0.1f°", channel.doubleValue] : @"----°"];
        ...
        if ( [channel isOnline] ) {
            ...
            switch([channel.func intValue]) {
                ...
                case SUPLA_CHANNELFNC_CONTROLLINGTHEROLLERSHUTTER:
                    ...
                    bl = [MGSwipeButton buttonWithTitle:NSLocalizedString(@"Cover", nil) icon:nil backgroundColor:[UIColor blackColor]];
                    break;
                case SUPLA_CHANNELFNC_POWERSWITCH:
                case SUPLA_CHANNELFNC_LIGHTSWITCH:
