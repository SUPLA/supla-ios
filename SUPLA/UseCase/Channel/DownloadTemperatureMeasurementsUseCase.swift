...
protocol DownloadTemperatureMeasurementsUseCase {
    func loadMeasurements(remoteId: Int32) -> Observable<Float>
}

final class DownloadTemperatureMeasurementsUseCaseImpl: BaseDownloadMeasurementsUseCase, DownloadTemperatureMeasurementsUseCase {
    ...
    func findCount(remoteId: Int32, profile: AuthProfileItem) -> Observable<Int> {
        temperatureMeasurementItemRepository.findCount(remoteId: remoteId, profile: profile)
    }
    
    func getMeasurements(remoteId: Int32, afterTimestamp: TimeInterval) -> Observable<[SuplaCloudClient.TemperatureMeasurement]> {
        service.getTemperatureMeasurements(remoteId: remoteId, afterTimestamp: afterTimestamp)
    }
    ...
    func storeMeasurements(measurements: [SuplaCloudClient.TemperatureMeasurement], timestamp: TimeInterval, profile: AuthProfileItem, remoteId: Int32) throws -> TimeInterval {
        try temperatureMeasurementItemRepository.storeMeasurements(measurements: measurements, timestamp: timestamp, profile: profile, remoteId: remoteId)
    }
