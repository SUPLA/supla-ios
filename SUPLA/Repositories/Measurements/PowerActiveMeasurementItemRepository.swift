...
protocol PowerActiveMeasurementItemRepository:
    BaseMeasurementRepository<SuplaCloudClient.HistoryMeasurement, SAPowerActiveMeasurementItem>, RemoveHiddenChannelsUseCaseImpl.Deletable where
    T == SAPowerActiveMeasurementItem
{
    func deleteAll(for serverId: Int32?) -> Observable<Void>
    func findMeasurements(remoteId: Int32, serverId: Int32?, phase: Phase, startDate: Date, endDate: Date) -> Observable<[SAPowerActiveMeasurementItem]>
}

final class PowerActiveMeasurementItemRepositoryImpl: Repository<SAPowerActiveMeasurementItem>, PowerActiveMeasurementItemRepository {
    
    @Singleton<SuplaCloudService> private var cloudService
    
    func deleteAll(for serverId: Int32?) -> Observable<Void> {
        deleteAll(
            SAPowerActiveMeasurementItem
                .fetchRequest()
    ...
    func findMeasurements(remoteId: Int32, serverId: Int32?, phase: Phase, startDate: Date, endDate: Date) -> Observable<[SAPowerActiveMeasurementItem]> {
        query(
            SAPowerActiveMeasurementItem.fetchRequest()
    ...
    func findMinTimestamp(remoteId: Int32, serverId: Int32?) -> Observable<TimeInterval?> {
        let request = SAPowerActiveMeasurementItem.fetchRequest()
            .filtered(by: NSPredicate(format: "channel_id = %d AND server_id = %d", remoteId, serverId ?? 0))
    ...
    func getInitialMeasurements(remoteId: Int32) -> Observable<(response: HTTPURLResponse, data: Data)> {
        cloudService.getInitialMeasurements(remoteId: remoteId, type: .powerActive)
    }
    
    func getMeasurements(remoteId: Int32, afterTimestamp: TimeInterval) -> Observable<[SuplaCloudClient.HistoryMeasurement]> {
        cloudService.getHistoryMeasurements(remoteId: remoteId, afterTimestamp: afterTimestamp, type: .powerActive)
    }
    ...
    func storeMeasurements(measurements: [SuplaCloudClient.HistoryMeasurement], timestamp: TimeInterval, serverId: Int32, remoteId: Int32) throws -> TimeInterval {
        ...
        context.performAndWait {
            for measurement in measurements {
                ...
                let entity: SAPowerActiveMeasurementItem = context.create()
    ...
    func deleteSync(_ remoteId: Int32, _ profile: AuthProfileItem) {
        ...
        context.performAndWait {
            let fetch = NSFetchRequest<NSFetchRequestResult>(entityName: "SAPowerActiveMeasurementItem")
