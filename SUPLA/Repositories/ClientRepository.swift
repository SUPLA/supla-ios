...
protocol ClientRepository {
    func users(email: String) -> Observable<Client.Autodiscover>
}
    ...
    func users(email: String) -> Observable<Client.Autodiscover> {
        guard let encodedEmail = email.urlEncoded()
            ...
            return Observable.error(
                ClientError.inputError(message: "Could not encode email address")
            )
        }
        ...
        return requestHelper.getRequest(urlString: urlString)
            .map { data in
                if let autodiscover = Client.Autodiscover(data: data) {
                    return autodiscover
                }
                
                throw ClientError.parseError(message: "Could not parse response data: '\(data)'")
            }
    }
}
